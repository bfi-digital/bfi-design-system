...
export const ShowPerformanceList = ({
        ...
        <Outer>
            ...
            <PerformanceList id="performance-list">
                {performances.map((performance) => {
                    let performanceTimeCheck =
            new Date(performance.dateTimeStart).getHours() === 0 &&
              new Date(performance.dateTimeStart).getMinutes() === 0
                ? false
                : true
                    return (
                        <Performance key={performance.id}>
                            <PerformanceDetails>
                                ...
                                {datesAreOnSameDayCheck(
                                    new Date(performance.dateTimeStart),
                                            ...
                                            <p>
                                                ...
                                                >
                                                    {performanceTimeCheck ? (
                                                        ...
                                                        <Moment tz="Europe/London" format="dddd D MMMM">
                                                            {performance.dateTimeStart}
                                                        </Moment>
                                                    )}
                                                </ConditionalWrapper>
    ...
    function getPerformanceInfo(performance) {
        ...
        performance.performanceInfo
            ...
            : performance.platform === "southbank"
                ? (info = (
                    <span>
                        {performance.paywall === "free" || performance.paywall === "Free"
                            ? "Screening for free "
                            : "Screening"}{" "}
            at {performance.screen ? performance.screen : "BFI Southbank"}
                    </span>
                ))
                    ...
                    ? (info = (
                        <span>
                            {performance.paywall === "free" || performance.paywall === "Free"
                                ? "Available for free "
                                : "Available"}{" "}
            on {performance.screen ? performance.screen : "BFI Player"}{" "}
                        </span>
                    ))
    ...
    function getPerformanceOnSaleDates(performance) {
        ...
        let showDate = false
        if (
            performance.on_sale_dates &&
      dayjs().isBefore(dayjs(priorityDate), dayjs(publicDate))
        ) {
            showDate = true
        }
        ...
        return showDate ? (
            <StyledDates>
                {priorityDate && (
                    <time dateTime={dayjs(priorityDate).unix()}>
            Priority booking opens{" "}
                        <Moment tz="Europe/London" format="dddd D MMMM - HH:mm">
                            {priorityDate}
                        </Moment>
                    </time>
                )}
                {publicDate && (
                    <time dateTime={dayjs(publicDate).unix()}>
    ...
    function getPerformanceButton(performance) {
        const betweenTimes = dayjs().isBetween(
            dayjs(performance.dateTimeStart),
            dayjs(performance.dateTimeEnd)
        )
        const performanceStarted = dayjs().isAfter(
            dayjs(performance.dateTimeStart)
        )
        ...
        if (performanceEnded) {
            ...
            button = (
                <StyledButton level={1} url={performance.ctaURL} external={true}>
          Book now
                </StyledButton>
            )
        } else if (performance.availability === "Sold out") {
            button = (
                <StyledButton level={1} external={false} disabled>
          Sold out
                </StyledButton>
            )
          ...
          Check back for tickets
            ...
            performance.ctaURL &&
      (performance.bookingRequired === "paid" ||
        performance.bookingRequired === "Paid")
        ) {
            ...
            performance.ctaURL &&
      (performance.bookingRequired === "free" ||
        performance.bookingRequired === "Free")
        ) {
            if (!performanceStarted) {
                button = (
